...
def load_restaurants_and_chunk_data(json_path):
    ...
    try:
        ...
        if not chunks:
            st.error("Failed to build data chunks from loaded restaurants.")
            return restaurants_data, None

        return restaurants_data, chunks

    except FileNotFoundError:
        st.error(f"Error: knowledgebase.json not found at {json_path}. Please ensure the file exists in the 'data' directory.")
        return None, None
        ...
        if prompt := st.chat_input("Ask your question here..."):
            ...
            with st.chat_message("assistant"):
                ...
                with st.spinner("Thinking..."):
                    response = answer_dynamic(prompt, embed_model, index, chunks, restaurants_data)
